# K Nearest Neighbor Classification (Car Evaluation)
...
for i in range(1, 15, 1):
    classifier = KNeighborsClassifier(n_neighbors = i, metric = 'matching')
...
plt.grid(True)
plt.show()
# Maximum accuracy occurs at K = 8

# Part C - Make predictions on the test data based on training model chosen

# Redo K-NN classifier with K = 8
classifier = KNeighborsClassifier(n_neighbors = 8, metric = 'matching')
scores = cross_val_score(classifier, X_train, y_train, cv=10)
print('Mean = ', np.mean(scores))
# Based on cross-validation, should predict with 92.3% accuracy
...
# Based on the test data, model predicts with 92.4% accuracy
# Misclassifies 11 as unacceptable that should have been acceptable or better
# Misclassifies 7 as acceptable that should have been unacceptable
...
plt.show()
# Training error rate degrades with test error rate at lower training sizes
# Test error rate degrades faster than training at lower training sizes
